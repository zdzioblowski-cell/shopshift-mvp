...
const available=PRODUCTS.map((_,i)=>i).filter(i=>s.stock[i].qty>0);
if(available.length){for(let n=0;n<s.visits;n++){const i=available[Math.floor(rng(s)*available.length)];const relative=s.prices[i]/PRODUCTS[i].price;const conversion=Math.min(.65,.24*Math.pow(1/relative,2.8));if(rng(s)>conversion)continue;if(s.stock[i].qty===0||s.orders.length>=LEVELS[s.level].limit){s.lost++;continue;}const st=s.stock[i];const cost=round(st.value/st.qty);st.qty--;st.value=st.qty?round(st.value-cost):0;s.orders.push({id:s.nextId++,index:i,price:s.prices[i],cost,status:'new',freeShipping:s.freeShipping,returnRoll:rng(s),quality:relative>1.3});}}
s.tutorial=Math.max(s.tutorial,3);log(s,'Sklep otwarty: '+s.visits+' odwiedzin, '+s.orders.length+' zamówień.');
if(s.worker){s.orders.slice(0,20).forEach(o=>o.status='packed');s.packedToday=Math.min(s.orders.length,20);log(s,'Magazynier spakował '+s.packedToday+' zamówień.');}
}
