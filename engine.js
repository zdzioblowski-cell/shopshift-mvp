...
const available=PRODUCTS.map((_,i)=>i).filter(i=>s.stock[i].qty>0||s.deliveries.some(d=>d.index===i)),remaining=s.stock.map((x,i)=>x.qty+s.deliveries.filter(d=>d.index===i).reduce((n,d)=>n+d.qty,0)),queue=[];b.channelStats=[];
...
remaining[index]-=o.qty;queue.push({...o,liveOffer:true,units,useCoupon,index,channel:source.id,freeShipping:s.freeShipping,returnRoll:rng(s),quality:relative>1.3});
}}
...
const delivery=Math.min(Infinity,...s.deliveries.filter(d=>d.remainingMs!==undefined).map(d=>d.remainingMs));const step=Math.min(ms,arrival,packing,delivery);if(!Number.isFinite(step))break;
if(live.queue.length)live.nextIn=Math.max(0,live.nextIn-step);if(live.job)live.job.elapsed+=step;ms-=step;for(const d of s.deliveries)if(d.remainingMs!==undefined)d.remainingMs=Math.max(0,d.remainingMs-step);for(const d of s.deliveries.filter(d=>d.remainingMs===0)){s.stock[d.index].qty+=d.qty;s.stock[d.index].value=round(s.stock[d.index].value+d.value);log(s,"Dostawa dotarła: "+d.qty+" × "+PRODUCTS[d.index].name+".");events.push({type:"delivery",delivery:{...d}});}s.deliveries=s.deliveries.filter(d=>d.remainingMs!==0);
...
assign();if(!live.queue.length&&!live.job&&!s.deliveries.some(d=>d.remainingMs!==undefined))break;
}
return events;
}
