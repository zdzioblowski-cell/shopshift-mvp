...
function migrate(s){for(const o of s.live?.queue||[]){if(o.qty!==undefined&&o.liveOffer===undefined){o.liveOffer=true;o.units=Math.min(3,o.qty);o.useCoupon=o.promo==='coupon'?o.discount>0:true;}}for(const d of s.deliveries){if(d.remainingMs===undefined){d.remainingMs=60000+Math.floor(rng(s)*240001);delete d.due;}} if(!s.business){s.business=defaults();s.business.budgets.google=CAMPAIGNS[s.campaign]?.cost||0;}if(!s.business.seoPending)s.business.seoPending=[];for(const l of [s.ledger,s.total,...s.history])for(const k of EXTRA_COSTS)if(l[k]===undefined)l[k]=0;return s;}
...
function price(s,index,value){if(!PRODUCTS[index]||!Number.isFinite(value)||value<5||value>300)throw Error('Cena musi wynosić 5–300 zł.');s.prices[index]=round(value);s.tutorial=Math.max(s.tutorial,2);}
...
function trafficTick(s){const t=s.traffic,b=s.business,live=ensureLive(s);t.elapsed=Math.min(t.duration,t.elapsed+1000);const minute=Math.min(2,Math.floor((t.elapsed-1)/60000));if(!t.minutes[minute])t.minutes[minute]={visits:0,orders:0};
for(const source of t.sources){const stats=b.channelStats.find(c=>c.id===source.id),target=Math.floor(source.planned*(t.elapsed-source.start)/(t.duration-source.start));
...
const liveOK=s=>s.live===undefined||!!(s.live&&arr(s.live.queue,160)&&s.live.queue.every(o=>o&&offerOK(o)&&integer(o.index,0,5)&&num(o.price,.01,1200)&&typeof o.freeShipping==='boolean'&&typeof o.quality==='boolean'&&num(o.returnRoll,0,1))&&num(s.live.nextIn,0,50000)&&(s.live.queue.length?s.live.nextIn>0:s.live.nextIn===0)&&(s.live.job===null||(s.live.job&&integer(s.live.job.id,1001)&&num(s.live.job.elapsed,0,s.worker?9000:16000))));
const trafficOK=t=>t===undefined||!!(t&&num(t.elapsed,0,180000)&&t.duration===180000&&num(t.nextIn,0,1000)&&((t.elapsed<t.duration&&t.nextIn>0)||t.elapsed===t.duration)&&arr(t.sources,6)&&t.sources.every(c=>['organic',...CHANNELS.map(x=>x.id)].includes(c.id)&&integer(c.planned,0,100000)&&integer(c.served,0,c.planned)&&num(c.start,0,179999)&&num(c.cvr,0,1)&&num(c.variation,0,2)&&num(c.overlap,0,1))&&arr(t.products,6)&&t.products.length===6&&t.products.every(c=>integer(c.views)&&integer(c.carts))&&arr(t.minutes,3)&&t.minutes.every(c=>integer(c.visits)&&integer(c.orders)));
return !!(s&&trafficOK(s.traffic)&&liveOK(s)&&businessOK(s.business)&&s.version===1&&integer(s.day,1)&&['plan','fulfill','report'].includes(s.phase)&&num(s.cash,-1e12)&&integer(s.level,0,2)&&integer(s.seed,0,4294967295)&&num(s.rating,1,5)&&integer(s.ratingCount,1)&&integer(s.shipped)&&integer(s.packedToday)&&integer(s.visits)&&integer(s.lost)&&integer(s.nextId,1001)&&integer(s.tutorial,0,5)&&typeof s.freeShipping==='boolean'&&typeof s.worker==='boolean'&&integer(s.campaign,0,3)&&arr(s.stock,6)&&s.stock.length===6&&s.stock.every(x=>x&&integer(x.qty,0,1200)&&num(x.value))&&arr(s.prices,6)&&s.prices.length===6&&s.prices.every(x=>num(x,5,300))&&arr(s.orders,160)&&s.orders.every(orderOK)&&arr(s.deliveries,240)&&s.deliveries.every(d=>d&&integer(d.index,0,5)&&integer(d.qty,1,1200)&&num(d.value)&&(d.remainingMs!==undefined?num(d.remainingMs,0,300000):integer(d.due,1)))&&arr(s.returns,320)&&s.returns.every(o=>orderOK(o)&&integer(o.due,1)&&num(o.sales))&&arr(s.reviews,25)&&s.reviews.every(r=>r&&integer(r.day,1)&&integer(r.stars,1,5)&&typeof r.text==='string'&&r.text.length<=1000)&&arr(s.log,12)&&s.log.every(x=>typeof x==='string'&&x.length<2000)&&arr(s.history,100000)&&s.history.every(h=>entryOK(h)&&integer(h.day,1)&&num(h.profit,-1e12)&&integer(h.orders)&&integer(h.visits)&&(h.channels===undefined||businessOK({...defaults(),channelStats:h.channels})))&&entryOK(s.ledger)&&entryOK(s.total));
}
return {reach,audience,conversion,boost,hasDemand,KNEES,PRODUCTS,LEVELS,CAMPAIGNS,CHANNELS,defaults,migrate,config,settings,spend,dayCosts,offer,unitEconomics,qty,create,profit,round,used,buy,price,open,advance,ensureLive,packingDuration,pack,ship,close,next,upgrade,hire,validate};
});
